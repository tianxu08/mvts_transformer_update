...
def load_from_tsfile_to_dataframe_sklearn(full_file_path_and_name,
    ...
    if line_num:
        ...
        for dim in range(0, num_dimensions):
            print('instance_list[dim]', instance_list[dim][0].shape)
            data['dim_' + str(dim)] = instance_list[dim]
...
def load_from_arff_to_dataframe_sklearn(full_file_path_and_name, has_class_labels=True,
                                return_separate_X_and_y=True,
                                replace_missing_vals_with='NaN'):
    instance_list = []
    class_val_list = []
...
def load_from_ucr_tsv_to_dataframe_sklearn(full_file_path_and_name,
                                   return_separate_X_and_y=True):
...
def from_long_to_nested_sklearn(long_dataframe):
...
def generate_example_long_table_sklearn(num_cases=50, series_len=20, num_dims=2):
